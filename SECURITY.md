...
This repository contains **documentation and reference material** about Flutter security practices. It does not ship executable code, a backend service, or a published package.

Security concerns in scope:

- **Incorrect security guidance** — a recommendation in the docs that would make a Flutter app *less* secure if followed (e.g., wrong SSL pinning implementation, insecure key storage pattern)
...
Documentation is maintained against the latest stable Flutter and Dart releases. We do not backport fixes to older versions.

| Flutter Version | Docs Status |
|---|---|
| 3.x (latest stable) | Actively maintained |
...
1. Go to the [Security tab](https://github.com/kasinadhsarma/flutter-skills/security) of this repository
...
```
[flutter-skills] Security: <brief description>
```
...
- Any references (CVEs, OWASP advisories, official Flutter/Dart docs)
...
We publicly credit reporters (by name or handle, at their preference) in the relevant commit message and changelog entry. Thank you for helping keep Flutter developers on the right path.
