...
3. **Working code only** — every code snippet must compile against a current stable Flutter/Dart version.
...
```bash
git clone https://github.com/kasinadhsarma/flutter-skills.git
cd flutter-skills
...
1. Fork the repository and create a branch: `fix/overflow-guide-intrinsicheight`
...
Small fixes (typos, broken code, outdated package names) don't need an issue first.  
...
A new guide is a markdown file in `docs/` covering a Flutter topic not already addressed.
...
- [ ] All code examples use proper fenced code blocks with a language tag (` ```dart `)
...
- [ ] Tested against current Flutter stable (run the code snippets)
...
Examples:
- `docs/Flutter Testing Strategies.md`
- `docs/Flutter Web Deployment Guide.md`
- `docs/Flutter Performance Profiling.md`
...
(Content — practical steps, code templates, decision tables)
```
...
- `graph TD` / `graph LR` — architecture and dependency flows
- `sequenceDiagram` — request/response and auth flows
- `flowchart LR` — development process steps
...
Open a [GitHub Discussion](https://github.com/kasinadhsarma/flutter-skills/discussions) or use the [new skill request issue template](.github/ISSUE_TEMPLATE/new_skill.md).
